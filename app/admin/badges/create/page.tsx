...
import { createBadge, type CreateBadgeRequest } from '@/lib/apiHelpers/adminBadges';
import { toast } from 'react-hot-toast';

export default function CreateBadgePage() {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState<CreateBadgeRequest>({
    name: '',
    description: '',
    type: '',
    rarity: '',
  ...
  const handleNestedChange = (parent: keyof CreateBadgeRequest, field: string, value: string | number) => {
    setFormData(prev => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Submitting badge data:', formData);
    // Validation
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Create New Badge</h1>
          <p className="text-muted-foreground mt-2">
            Create a new badge for the system
          </p>
        </div>
        <Button
          variant="outline"
