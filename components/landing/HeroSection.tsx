...
export default function HeroSection() {
  ...
  const { ref: subtitleRef, isVisible: subtitleVisible } = useScrollAnimationWithDelay(200);
  const { ref: buttonsRef, isVisible: buttonsVisible } = useScrollAnimationWithDelay(400);
  const { ref: statsRef, isVisible: statsVisible } = useScrollAnimationWithDelay(600);
    ...
    <section className="relative z-10 px-6 py-20">
      <div className="max-w-7xl mx-auto text-center">
        ...
        <motion.div
          ...
          animate={subtitleVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
          transition={{ duration: 0.6, ease: "easeOut" }}
          className="mb-8"
        >
          <p className="text-xl md:text-2xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Upload your EV odometer photos and earn{' '}
            <span className="text-gradient-primary font-semibold">B3TR tokens</span> via{' '}
            <span className="text-gradient-success font-semibold">VeChain blockchain</span>. 
        ...
        <motion.div
          ref={buttonsRef}
          initial={{ opacity: 0, y: 30 }}
          animate={buttonsVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
          transition={{ duration: 0.6, ease: "easeOut" }}
          ...
          >
            <Card className="hover-lift gradient-ev-cream/10 border-warning/20 backdrop-blur-sm">
              <CardContent className="p-8 text-center">
                <h3 className="text-lg text-muted-foreground mb-4">Rewards Distributed</h3>
                                 <div className="flex justify-center items-center space-x-2 mb-4">
                   {formatOdometerNumber(125000).split('').map((digit: string, index: number) => (
                     <div key={index} className="w-12 h-16 bg-black rounded-md flex items-center justify-center">
                       <span className="odometer-digit text-2xl font-mono">{digit}</span>
                     </div>
                   ))}
                 </div>
                <p className="text-sm text-muted-foreground">Total rewards distributed to our community</p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            >
              <Card className="hover-lift gradient-ev-green/10 border-primary/20 backdrop-blur-sm">
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-gradient-ev-green mb-1">2.5M</div>
                  <div className="text-xs text-muted-foreground">Total Carbon Impact</div>
                </CardContent>
              </Card>
            </motion.div>
            ...
            >
              <Card className="hover-lift gradient-ev-light/10 border-success/20 backdrop-blur-sm">
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-gradient-ev-light mb-1">15.2K</div>
                  <div className="text-xs text-muted-foreground">EV Miles Tracked</div>
                </CardContent>
              </Card>
            </motion.div>
